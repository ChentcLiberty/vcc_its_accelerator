module its_1d_core #(
    ...
    localparam integer ROM_WORDS   = TR_COUNT * TABLE_COUNT * TABLE_WORDS;
    ...
    function is_supported_cfg;
        input [1:0] tr_type_in;
        input [6:0] size_in;
        begin
            case (tr_type_in)
                TR_DCT2: begin
                    case (size_in)
                        ...
                        7'd16,
                        7'd32,
                        7'd64: is_supported_cfg = 1'b1;
                        default: is_supported_cfg = 1'b0;
                    endcase
                end
                TR_DST7,
                TR_DCT8: begin
    ...
    function [3:0] calc_table_sel;
        input [1:0] tr_type_in;
        input [6:0] size_in;
        reg [3:0] base_idx;
        begin
            case (tr_type_in)
                TR_DCT2: base_idx = 4'd0;
                TR_DST7: base_idx = 4'd5;
                TR_DCT8: base_idx = 4'd10;
                default: base_idx = 4'd0;
            endcase

            case (size_in)
                7'd4:  calc_table_sel = base_idx + 4'd0;
                7'd8:  calc_table_sel = base_idx + 4'd1;
                7'd16: calc_table_sel = base_idx + 4'd2;
                7'd32: calc_table_sel = base_idx + 4'd3;
                7'd64: calc_table_sel = base_idx + 4'd4;
                default: calc_table_sel = base_idx;
            endcase
        end
    endfunction
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!busy_r) begin
                if (start && is_supported_cfg(tr_type, n_tbs)) begin
                    busy_r          <= 1'b1;
                    out_valid_r     <= 1'b0;
                    size_r          <= n_tbs;
                    row_base_r      <= 7'd0;
                    non_zero_size_r <= clamp_non_zero_size(non_zero_size, n_tbs);
                    table_sel_r     <= calc_table_sel(tr_type, n_tbs);
