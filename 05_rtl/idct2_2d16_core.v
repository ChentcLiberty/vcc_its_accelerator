module idct2_2d16_core #(
    ...
    input  wire signed [DATA_W-1:0]   x_in [0:255],
    input  wire                       out_req,
    output wire                       out_valid,
    output wire                       out_last,
    output wire [7:0]                 out_index_base,
    ...
    localparam [3:0] S_PREP_COL_0  = 4'd4;
    localparam [3:0] S_PREP_COL_1  = 4'd5;
    localparam [3:0] S_PREP_COL_2  = 4'd6;
    localparam [3:0] S_PREP_COL_3  = 4'd7;
    localparam [3:0] S_START_COL   = 4'd8;
    localparam [3:0] S_RUN_COL     = 4'd9;
    localparam [3:0] S_STREAM_OUT  = 4'd10;
    ...
    reg signed [DATA_W-1:0] x_in_r [0:255];
    ...
    assign out_last = (state_r == S_STREAM_OUT) && (out_group_r == 6'd63);
    assign out_index_base = ({4'd0, out_group_r[5:2]} << 4) + {4'd0, out_group_r[1:0], 2'b00};

    assign mid_clear = (state_r == S_IDLE) && start;
    assign final_clear = (state_r == S_IDLE) && start;

    assign mid_rd_minor_base =
        (state_r == S_PREP_COL_0) ? 7'd0  :
        (state_r == S_PREP_COL_1) ? 7'd4  :
        (state_r == S_PREP_COL_2) ? 7'd8  :
                                     7'd12;
    assign final_rd_row_idx = {3'd0, out_group_r[5:2]};
    assign final_rd_col_base = {3'd0, out_group_r[1:0], 2'b00};
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            col_idx_r       <= 4'd0;
            out_group_r     <= 6'd0;
            non_zero_cols_r <= 7'd16;
            non_zero_rows_r <= 7'd16;
            done            <= 1'b0;
            ...
            case (state_r)
                ...
                S_PREP_ROW: begin
                    ...
                    for (idx = 0; idx < 16; idx = idx + 1) begin
                        row_x_r[idx] <= $signed(x_in_r[(row_idx_r * 16) + idx]);
                    end
                    state_r <= S_START_ROW;
                end
                ...
                S_RUN_ROW: begin
                    if (row_done) begin
                        if (row_idx_r == 4'd15) begin
                            col_idx_r <= 4'd0;
                            state_r <= S_PREP_COL_0;
                        end else begin
                            row_idx_r <= row_idx_r + 4'd1;
                            state_r <= S_PREP_ROW;
                        end
                    end
                end
                ...
                S_PREP_COL_1: begin
                    ...
                    state_r <= S_PREP_COL_2;
                end

                S_PREP_COL_2: begin
                    col_x_r[8]  <= mid_rd_data_0;
                    col_x_r[9]  <= mid_rd_data_1;
                    col_x_r[10] <= mid_rd_data_2;
                    col_x_r[11] <= mid_rd_data_3;
                    state_r <= S_PREP_COL_3;
                end

                S_PREP_COL_3: begin
                    col_x_r[12] <= mid_rd_data_0;
                    col_x_r[13] <= mid_rd_data_1;
                    col_x_r[14] <= mid_rd_data_2;
                    col_x_r[15] <= mid_rd_data_3;
                    state_r <= S_START_COL;
                end
                ...
                S_RUN_COL: begin
                    if (col_done) begin
                        if (col_idx_r == 4'd15) begin
                            out_group_r <= 6'd0;
                            state_r <= S_STREAM_OUT;
                        end else begin
                            col_idx_r <= col_idx_r + 4'd1;
                            state_r <= S_PREP_COL_0;
                        end
                    end
                end
                ...
                S_STREAM_OUT: begin
                    if (out_req) begin
                        if (out_group_r == 6'd63) begin
                            done <= 1'b1;
                            state_r <= S_IDLE;
                        end else begin
                            out_group_r <= out_group_r + 6'd1;
                        end
                    end
                end
