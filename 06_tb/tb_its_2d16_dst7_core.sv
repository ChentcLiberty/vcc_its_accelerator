    ...
    task automatic run_case(
        ...
        begin
            ...
            while (accepted_groups < 64) begin
                @(posedge clk);
                if (out_valid === 1'b1 && out_req === 1'b1) begin
                    if (out_index_base !== (accepted_groups * 4)) begin
                        $display("ERR %s base got %0d exp %0d", case_name, out_index_base, accepted_groups * 4);
                        error_count = error_count + 1;
                    end

                    if (out_last !== (accepted_groups == 63)) begin
                        $display("ERR %s out_last at group %0d", case_name, accepted_groups);
                        error_count = error_count + 1;
                    end

                    check_group(case_name, accepted_groups * 4, expected);

                    if (accepted_groups == stall_group) begin
                        out_req = 1'b0;
                        #1;
                        hold_base = out_index_base;
                        hold_0 = out_data_0;
                        hold_1 = out_data_1;
                        hold_2 = out_data_2;
                        hold_3 = out_data_3;
                        ...
                        #1;
                        if (out_valid !== 1'b1 ||
                            out_index_base !== hold_base ||
                            out_data_0 !== hold_0 ||
                            out_data_1 !== hold_1 ||
                            out_data_2 !== hold_2 ||
                            out_data_3 !== hold_3) begin
                            $display(
                                "ERR %s out hold mismatch during stall base=%0d/%0d d0=%0d/%0d d1=%0d/%0d d2=%0d/%0d d3=%0d/%0d",
                                case_name,
                                out_index_base, hold_base,
                                out_data_0, hold_0,
                                out_data_1, hold_1,
